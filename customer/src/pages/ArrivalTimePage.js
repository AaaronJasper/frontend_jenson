...
function getFutureTimes() {
  const now = new Date();
  const currentMinutes = now.getHours() * 60 + now.getMinutes();
  return availableArrivalTimes.filter(time => {
...
function ArrivalTimePage() {
  ...
  const futureTimes = getFutureTimes();
  const initialTime = arrivalTime && futureTimes.includes(arrivalTime) ? arrivalTime : '';
    ...
    <main className="arrival-time-page">
      ...
      <section className="arrival-time-card">
        ...
        {futureTimes.length === 0 ? (
          <p className="no-times-message">No available arrival times for today. Please try again tomorrow.</p>
        ) : (
        ...
        <button
          ...
          disabled={!selectedTime || futureTimes.length === 0}
        >
          Continue to Menu
        </button>
      </section>
